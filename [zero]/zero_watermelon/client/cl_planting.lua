...
RegisterNetEvent('zero-watermelon:client:UseWeedSeed', function()
        ...
        if hit == 1 then
            ...
            if IsControlJustPressed(0, 38) then
                ...
                QBCore.Functions.Progressbar('spawn_plant', _U('place_sapling'), 2000, false, true, {
                    ...
                    TriggerServerEvent('zero-watermelon:server:CreateNewPlant', dest)
                    planted = false
                    seedUsed = false
                    ClearPedTasks(ped)
...
RegisterNetEvent('zero-watermelon:client:CheckPlant', function(data)
    local netId = NetworkGetNetworkIdFromEntity(data.entity)
    QBCore.Functions.TriggerCallback('zero-watermelon:server:GetPlantData', function(result)
        if not result then return end
        if result.health == 0 then -- Destroy plant
            exports['qb-menu']:openMenu({
                {
                    header = 'Watermelon Plant',
                    txt = _U('esc_to_close'),
                ...
                },
                {
                    ...
                    params = {
                        event = 'zero-watermelon:client:ClearPlant',
                        args = data.entity
                    }
                }
            })
        elseif result.growth == 100 then -- Harvest
                exports['qb-menu']:openMenu({
                    ...
                    },
                    {
                        ...
                        params = {
                            event = 'zero-watermelon:client:HarvestPlant',
                            args = data.entity
                        }
                    },
                })
        elseif result.gender == 'female' then -- Option to add male seed
                exports['qb-menu']:openMenu({
                    ...
                    },
                    {
                        ...
                        icon = 'fas fa-shower',
                        params = {
                            event = 'zero-watermelon:client:GiveWater',
                            args = data.entity
                        }
                    },
                    {
                        ...
                        params = {
                            args = data.entity,
                            event = 'zero-watermelon:client:GiveFertilizer',
                        }
                    }
                })
...
RegisterNetEvent('zero-watermelon:client:ClearPlant', function(entity)
    ...
    QBCore.Functions.Progressbar('clear_plant', _U('clear_plant'), 8500, false, true, {
        ...
        TriggerServerEvent('zero-watermelon:server:ClearPlant', netId)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    end, function()
...
RegisterNetEvent('zero-watermelon:client:HarvestPlant', function(entity)
    local netId = NetworkGetNetworkIdFromEntity(entity)
    local ped = PlayerPedId()
    TaskTurnPedToFaceEntity(ped, entity, 1.0)
    Wait(1500)
    ...
    QBCore.Functions.Progressbar('harvest_plant', _U('harvesting_plant'), 8500, false, true, {
        ...
        TriggerServerEvent('zero-watermelon:server:HarvestPlant', netId)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    end, function()
...
RegisterNetEvent('zero-watermelon:client:PoliceDestroy', function(entity)
    local netId = NetworkGetNetworkIdFromEntity(entity)
    local ped = PlayerPedId()
    TaskTurnPedToFaceEntity(ped, entity, 1.0)
    Wait(500)
    ClearPedTasks(ped)
    TriggerServerEvent('zero-watermelon:server:PoliceDestroy', netId)
end)

RegisterNetEvent('zero-watermelon:client:FireGoBrrrrrrr', function(coords)
    local ped = PlayerPedId()
...
RegisterNetEvent('zero-watermelon:client:GiveWater', function(entity)
    if QBCore.Functions.HasItem(Shared.FullCanItem, 1) then
        ...
        QBCore.Functions.Progressbar('weedplanting_water', _U('watering_plant'), 6000, false, false, {
            ...
            TriggerServerEvent('zero-watermelon:server:GiveWater', netId)
            ClearPedTasks(ped)
...
RegisterNetEvent('zero-watermelon:client:OpenFillWaterMenu', function()
    exports['qb-menu']:openMenu({
        {
        ...
        },
        {
            ...
            params = {
                event = 'zero-watermelon:client:FillWater',
            }
        }
    })
end)

RegisterNetEvent('zero-watermelon:client:FillWater', function()
    ...
    QBCore.Functions.Progressbar('filling_water', _U('filling_water'), 2000, false, true, {
        ...
        TriggerServerEvent('zero-watermelon:server:GetFullWateringCan')
    end, function() -- Cancel
        QBCore.Functions.Notify(_U('canceled'), 'error', 2500)
    end)
end)

RegisterNetEvent('zero-watermelon:client:GiveFertilizer', function(entity)
    if QBCore.Functions.HasItem(Shared.FertilizerItem, 1) then
        ...
        QBCore.Functions.Progressbar('weedplanting_fertilizer', _U('fertilizing_plant'), 6000, false, false, {
            ...
            TriggerServerEvent('zero-watermelon:server:GiveFertilizer', netId)
            ClearPedTasks(ped)
            DeleteEntity(created_object)
        end, function()
            ClearPedTasks(ped)
...
RegisterNetEvent('zero-watermelon:client:AddMaleSeed', function(entity)
    if QBCore.Functions.HasItem(Shared.MaleSeed, 1) then
        ...
        QBCore.Functions.Progressbar('add_maleseed', _U('adding_male_seed'), 8500, false, true, {
            ...
            TriggerServerEvent('zero-watermelon:server:AddMaleSeed', netId)
            ClearPedTasks(ped)
            RemoveAnimDict('amb@medic@standing@kneel@base')
            RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
        end, function()
...
CreateThread(function()
    exports['qb-target']:AddTargetModel(Shared.WeedProps, {
        options = {
            {
                type = 'client',
                event = 'zero-watermelon:client:CheckPlant',
