...
local cucumberplants = {}

--- Functions

--- Method to calculate the growth percentage for a given cucumberplants index
--- @param k number - cucumberplants table index
--- @return retval number - growth index [0-100]
local calcGrowth = function(k)
    if not cucumberplants[k] then return false end
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
    local progress = os.difftime(current_time, cucumberplants[k].time)
...
--- Method to calculate the current fertilizer percentage for a given cucumberplants index
--- @param k number - cucumberplants table index
...
local calcFertilizer = function(k)
    ...
    if #cucumberplants[k].fertilizer == 0 then
        return 0
    else
        local last_fertilizer = cucumberplants[k].fertilizer[#cucumberplants[k].fertilizer]
...
--- Method to calculate the current water percentage for a given cucumberplants index
--- @param k number - cucumberplants table index
...
local calcWater = function(k)
    ...
    if #cucumberplants[k].water == 0 then
        return 0
    else
        local last_water = cucumberplants[k].water[#cucumberplants[k].water]
...
--- Method to calculate the health percentage for a given cucumberplants index
--- @param k number - cucumberplants table index
...
local calcHealth = function(k)
    ...
    local health = 100
    local current_time = os.time()
    local planted_time = cucumberplants[k].time
    ...
    for i=1, intervals, 1 do
        ...
        if #cucumberplants[k].fertilizer == 0 then
            ...
            for i=1, #cucumberplants[k].fertilizer, 1 do
                last_fertilizer = last_fertilizer < cucumberplants[k].fertilizer[i] and last_fertilizer or cucumberplants[k].fertilizer[i]
            end
        ...
        if #cucumberplants[k].water == 0 then
            ...
            for i=1, #cucumberplants[k].water, 1 do
                last_water = last_water < cucumberplants[k].water[i] and last_water or cucumberplants[k].water[i]
            end
...
--- Method to setup all the cucumberplants, fetched from the database
--- @return nil
local setupPlants = function()
    local result = MySQL.Sync.fetchAll('SELECT * FROM cucumberplants')
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
...
local destroyAllPlants = function()    
    for k, v in pairs(cucumberplants) do
        if DoesEntityExist(k) then
            DeleteEntity(k)
            cucumberplants[k] = nil
        end
    end
end
...
local updatePlantProp = function(k, stage)
    if not cucumberplants[k] then return end
    if not DoesEntityExist(k) then return end
    local ModelHash = Shared.WeedProps[stage]
    DeleteEntity(k)
    local plant = CreateObjectNoOffset(ModelHash, cucumberplants[k].coords.x, cucumberplants[k].coords.y, cucumberplants[k].coords.z + Shared.ObjectZOffset, true, true, false)
    FreezeEntityPosition(plant, true)
    cucumberplants[plant] = cucumberplants[k]
    cucumberplants[k] = nil
end
    ...
    if Shared.ClearOnStartup then
        ...
        for k, v in pairs(cucumberplants) do
            if calcHealth(k) == 0 then
                DeleteEntity(k)
                MySQL.query('DELETE from cucumberplants WHERE id = :id', {
                    ['id'] = cucumberplants[k].id
                })
                cucumberplants[k] = nil
            end
        end
...
RegisterNetEvent('zero-eggplant:server:ClearPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cucumberplants[entity] then return end
    if #(GetEntityCoords(GetPlayerPed(source)) - cucumberplants[entity].coords) > 10 then return end
    if calcHealth(entity) ~= 0 then return end

    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        MySQL.query('DELETE from cucumberplants WHERE id = :id', {
            ['id'] = cucumberplants[entity].id
        })
        cucumberplants[entity] = nil
    end
end)

RegisterNetEvent('zero-eggplant:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cucumberplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(GetEntityCoords(GetPlayerPed(src)) - cucumberplants[entity].coords) > 10 then return end
    if calcGrowth(entity) ~= 100 then return end

    if DoesEntityExist(entity) then
        local health = calcHealth(entity)
        if cucumberplants[entity].gender == 'female' then
...
-- RegisterNetEvent('zero-eggplant:server:PoliceDestroy', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not cucumberplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         TriggerClientEvent('zero-eggplant:client:FireGoBrrrrrrr', -1, cucumberplants[entity].coords)
...
RegisterNetEvent('zero-eggplant:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cucumberplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        ...
        cucumberplants[entity].water[#cucumberplants[entity].water + 1] = os.time()
        MySQL.update('UPDATE cucumberplants SET water = (:water) WHERE id = (:id)', {
            ['water'] = json.encode(cucumberplants[entity].water),
            ['id'] = cucumberplants[entity].id,
        })
    end
end)

RegisterNetEvent('zero-eggplant:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cucumberplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        ...
        cucumberplants[entity].fertilizer[#cucumberplants[entity].fertilizer + 1] = os.time()
        MySQL.update('UPDATE cucumberplants SET fertilizer = (:fertilizer) WHERE id = (:id)', {
            ['fertilizer'] = json.encode(cucumberplants[entity].fertilizer),
            ['id'] = cucumberplants[entity].id,
        })
    end
end)

-- RegisterNetEvent('zero-eggplant:server:AddMaleSeed', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not cucumberplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         cucumberplants[entity].gender = 'male'
--         MySQL.update('UPDATE cucumberplants SET gender = (:gender) WHERE id = (:id)', {
--             ['gender'] = cucumberplants[entity].gender,
--             ['id'] = cucumberplants[entity].id,
--         })
--     end
-- end)

RegisterNetEvent('zero-eggplant:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) then
        ...
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into cucumberplants (coords, time, fertilizer, water, gender) VALUES (:coords, :time, :fertilizer, :water, :gender)', {
...
RegisterNetEvent('zero-eggplant:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('zero-eggplant:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cucumberplants[entity] then cb(nil) return end
    local temp = {
        id = cucumberplants[entity].id,
        coords = cucumberplants[entity].coords,
        time = cucumberplants[entity].time,
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("zero-eggplant:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("zero-eggplant:client:OpenFillWaterMenu", src)
end)
