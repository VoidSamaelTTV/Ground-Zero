...
local eggplantplants = {}

--- Functions

--- Method to calculate the growth percentage for a given eggplantplants index
--- @param k number - eggplantplants table index
--- @return retval number - growth index [0-100]
local calcGrowth = function(k)
    if not eggplantplants[k] then return false end
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
    local progress = os.difftime(current_time, eggplantplants[k].time)
...
--- Method to calculate the current fertilizer percentage for a given eggplantplants index
--- @param k number - eggplantplants table index
...
local calcFertilizer = function(k)
    ...
    if #eggplantplants[k].fertilizer == 0 then
        return 0
    else
        local last_fertilizer = eggplantplants[k].fertilizer[#eggplantplants[k].fertilizer]
...
--- Method to calculate the current water percentage for a given eggplantplants index
--- @param k number - eggplantplants table index
...
local calcWater = function(k)
    ...
    if #eggplantplants[k].water == 0 then
        return 0
    else
        local last_water = eggplantplants[k].water[#eggplantplants[k].water]
...
--- Method to calculate the health percentage for a given eggplantplants index
--- @param k number - eggplantplants table index
...
local calcHealth = function(k)
    ...
    local health = 100
    local current_time = os.time()
    local planted_time = eggplantplants[k].time
    ...
    for i=1, intervals, 1 do
        ...
        if #eggplantplants[k].fertilizer == 0 then
            ...
            for i=1, #eggplantplants[k].fertilizer, 1 do
                last_fertilizer = last_fertilizer < eggplantplants[k].fertilizer[i] and last_fertilizer or eggplantplants[k].fertilizer[i]
            end
        ...
        if #eggplantplants[k].water == 0 then
            ...
            for i=1, #eggplantplants[k].water, 1 do
                last_water = last_water < eggplantplants[k].water[i] and last_water or eggplantplants[k].water[i]
            end
...
--- Method to setup all the eggplantplants, fetched from the database
--- @return nil
local setupPlants = function()
    local result = MySQL.Sync.fetchAll('SELECT * FROM eggplantplants')
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
...
local destroyAllPlants = function()    
    for k, v in pairs(eggplantplants) do
        if DoesEntityExist(k) then
            DeleteEntity(k)
            eggplantplants[k] = nil
        end
    end
end
...
local updatePlantProp = function(k, stage)
    if not eggplantplants[k] then return end
    if not DoesEntityExist(k) then return end
    local ModelHash = Shared.WeedProps[stage]
    DeleteEntity(k)
    local plant = CreateObjectNoOffset(ModelHash, eggplantplants[k].coords.x, eggplantplants[k].coords.y, eggplantplants[k].coords.z + Shared.ObjectZOffset, true, true, false)
    FreezeEntityPosition(plant, true)
    eggplantplants[plant] = eggplantplants[k]
    eggplantplants[k] = nil
end
    ...
    if Shared.ClearOnStartup then
        ...
        for k, v in pairs(eggplantplants) do
            if calcHealth(k) == 0 then
                DeleteEntity(k)
                MySQL.query('DELETE from eggplantplants WHERE id = :id', {
                    ['id'] = eggplantplants[k].id
                })
                eggplantplants[k] = nil
            end
        end
...
RegisterNetEvent('zero-eggplant:server:ClearPlant', function(netId)
    ...
    if not eggplantplants[entity] then return end
    if #(GetEntityCoords(GetPlayerPed(source)) - eggplantplants[entity].coords) > 10 then return end
    if calcHealth(entity) ~= 0 then return end

    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        MySQL.query('DELETE from eggplantplants WHERE id = :id', {
            ['id'] = eggplantplants[entity].id
        })
        eggplantplants[entity] = nil
    end
end)
...
RegisterNetEvent('zero-eggplant:server:HarvestPlant', function(netId)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - eggplantplants[entity].coords) > 10 then return end
    if calcGrowth(entity) ~= 100 then return end

    if DoesEntityExist(entity) then
        local health = calcHealth(entity)
        if eggplantplants[entity].gender == 'female' then
...
--         TriggerClientEvent('zero-eggplant:client:FireGoBrrrrrrr', -1, eggplantplants[entity].coords)
...
RegisterNetEvent('zero-eggplant:server:GiveWater', function(netId)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        ...
        eggplantplants[entity].water[#eggplantplants[entity].water + 1] = os.time()
        MySQL.update('UPDATE eggplantplants SET water = (:water) WHERE id = (:id)', {
            ['water'] = json.encode(eggplantplants[entity].water),
            ['id'] = eggplantplants[entity].id,
        })
    end
end)
...
RegisterNetEvent('zero-eggplant:server:GiveFertilizer', function(netId)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        ...
        eggplantplants[entity].fertilizer[#eggplantplants[entity].fertilizer + 1] = os.time()
        MySQL.update('UPDATE eggplantplants SET fertilizer = (:fertilizer) WHERE id = (:id)', {
            ['fertilizer'] = json.encode(eggplantplants[entity].fertilizer),
            ['id'] = eggplantplants[entity].id,
        })
    end
end)
...
--         eggplantplants[entity].gender = 'male'
--         MySQL.update('UPDATE eggplantplants SET gender = (:gender) WHERE id = (:id)', {
--             ['gender'] = eggplantplants[entity].gender,
--             ['id'] = eggplantplants[entity].id,
--         })
--     end
-- end)
...
RegisterNetEvent('zero-eggplant:server:CreateNewPlant', function(coords)
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) then
        ...
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into eggplantplants (coords, time, fertilizer, water, gender) VALUES (:coords, :time, :fertilizer, :water, :gender)', {
...
QBCore.Functions.CreateCallback('zero-eggplant:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not eggplantplants[entity] then cb(nil) return end
    local temp = {
        id = eggplantplants[entity].id,
        coords = eggplantplants[entity].coords,
        time = eggplantplants[entity].time,
