...
local cocaine_plants = {}

--- Functions

--- Method to calculate the growth percentage for a given cocaine_plants index
--- @param k number - cocaine_plants table index
--- @return retval number - growth index [0-100]
local calcGrowth = function(k)
    if not cocaine_plants[k] then return false end
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
    local progress = os.difftime(current_time, cocaine_plants[k].time)
...
--- Method to calculate the current fertilizer percentage for a given cocaine_plants index
--- @param k number - cocaine_plants table index
...
local calcFertilizer = function(k)
    ...
    if #cocaine_plants[k].fertilizer == 0 then
        return 0
    else
        local last_fertilizer = cocaine_plants[k].fertilizer[#cocaine_plants[k].fertilizer]
...
--- Method to calculate the current water percentage for a given cocaine_plants index
--- @param k number - cocaine_plants table index
...
local calcWater = function(k)
    ...
    if #cocaine_plants[k].water == 0 then
        return 0
    else
        local last_water = cocaine_plants[k].water[#cocaine_plants[k].water]
...
--- Method to calculate the health percentage for a given cocaine_plants index
--- @param k number - cocaine_plants table index
...
local calcHealth = function(k)
    ...
    local health = 100
    local current_time = os.time()
    local planted_time = cocaine_plants[k].time
    ...
    for i=1, intervals, 1 do
        ...
        if #cocaine_plants[k].fertilizer == 0 then
            ...
            for i=1, #cocaine_plants[k].fertilizer, 1 do
                last_fertilizer = last_fertilizer < cocaine_plants[k].fertilizer[i] and last_fertilizer or cocaine_plants[k].fertilizer[i]
            end
        ...
        if #cocaine_plants[k].water == 0 then
            ...
            for i=1, #cocaine_plants[k].water, 1 do
                last_water = last_water < cocaine_plants[k].water[i] and last_water or cocaine_plants[k].water[i]
            end
...
--- Method to setup all the cocaine_plants, fetched from the database
--- @return nil
local setupPlants = function()
    local result = MySQL.Sync.fetchAll('SELECT * FROM cocaine_plants')
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
...
local destroyAllPlants = function()    
    for k, v in pairs(cocaine_plants) do
        if DoesEntityExist(k) then
            DeleteEntity(k)
            cocaine_plants[k] = nil
        end
    end
end
...
local updatePlantProp = function(k, stage)
    if not cocaine_plants[k] then return end
    if not DoesEntityExist(k) then return end
    local ModelHash = Shared.WeedProps[stage]
    DeleteEntity(k)
    local plant = CreateObjectNoOffset(ModelHash, cocaine_plants[k].coords.x, cocaine_plants[k].coords.y, cocaine_plants[k].coords.z + Shared.ObjectZOffset, true, true, false)
    FreezeEntityPosition(plant, true)
    cocaine_plants[plant] = cocaine_plants[k]
    cocaine_plants[k] = nil
end
    ...
    if Shared.ClearOnStartup then
        ...
        for k, v in pairs(cocaine_plants) do
            if calcHealth(k) == 0 then
                DeleteEntity(k)
                MySQL.query('DELETE from cocaine_plants WHERE id = :id', {
                    ['id'] = cocaine_plants[k].id
                })
                cocaine_plants[k] = nil
            end
        end
...
RegisterNetEvent('zero-cocaine:server:ClearPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cocaine_plants[entity] then return end
    if #(GetEntityCoords(GetPlayerPed(source)) - cocaine_plants[entity].coords) > 10 then return end
    if calcHealth(entity) ~= 0 then return end

    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        MySQL.query('DELETE from cocaine_plants WHERE id = :id', {
            ['id'] = cocaine_plants[entity].id
        })
        cocaine_plants[entity] = nil
    end
end)

RegisterNetEvent('zero-cocaine:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cocaine_plants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(GetEntityCoords(GetPlayerPed(src)) - cocaine_plants[entity].coords) > 10 then return end
    if calcGrowth(entity) ~= 100 then return end

    if DoesEntityExist(entity) then
        local health = calcHealth(entity)
        if cocaine_plants[entity].gender == 'female' then
            local info = { health = health }
            Player.Functions.AddItem(Shared.BranchItem, math.random(4,16), false, info)
...
-- RegisterNetEvent('zero-cocaine:server:PoliceDestroy', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not cocaine_plants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         TriggerClientEvent('zero-cocaine:client:FireGoBrrrrrrr', -1, cocaine_plants[entity].coords)
...
RegisterNetEvent('zero-cocaine:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cocaine_plants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        ...
        cocaine_plants[entity].water[#cocaine_plants[entity].water + 1] = os.time()
        MySQL.update('UPDATE cocaine_plants SET water = (:water) WHERE id = (:id)', {
            ['water'] = json.encode(cocaine_plants[entity].water),
            ['id'] = cocaine_plants[entity].id,
        })
    end
end)

RegisterNetEvent('zero-cocaine:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cocaine_plants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        ...
        cocaine_plants[entity].fertilizer[#cocaine_plants[entity].fertilizer + 1] = os.time()
        MySQL.update('UPDATE cocaine_plants SET fertilizer = (:fertilizer) WHERE id = (:id)', {
            ['fertilizer'] = json.encode(cocaine_plants[entity].fertilizer),
            ['id'] = cocaine_plants[entity].id,
        })
    end
end)

-- RegisterNetEvent('zero-cocaine:server:AddMaleSeed', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not cocaine_plants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         cocaine_plants[entity].gender = 'male'
--         MySQL.update('UPDATE cocaine_plants SET gender = (:gender) WHERE id = (:id)', {
--             ['gender'] = cocaine_plants[entity].gender,
--             ['id'] = cocaine_plants[entity].id,
--         })
--     end
-- end)

RegisterNetEvent('zero-cocaine:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) then
        ...
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into cocaine_plants (coords, time, fertilizer, water, gender) VALUES (:coords, :time, :fertilizer, :water, :gender)', {
...
RegisterNetEvent('zero-cocaine:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('zero-cocaine:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cocaine_plants[entity] then cb(nil) return end
    local temp = {
        id = cocaine_plants[entity].id,
        coords = cocaine_plants[entity].coords,
        time = cocaine_plants[entity].time,
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("zero-cocaine:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("zero-cocaine:client:OpenFillWaterMenu", src)
end)
