...
local grapeape_plant = {}

--- Functions

--- Method to calculate the growth percentage for a given grapeape_plant index
--- @param k number - grapeape_plant table index
--- @return retval number - growth index [0-100]
local calcGrowth = function(k)
    if not grapeape_plant[k] then return false end
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
    local progress = os.difftime(current_time, grapeape_plant[k].time)
...
--- Method to calculate the current fertilizer percentage for a given grapeape_plant index
--- @param k number - grapeape_plant table index
...
local calcFertilizer = function(k)
    ...
    if #grapeape_plant[k].fertilizer == 0 then
        return 0
    else
        local last_fertilizer = grapeape_plant[k].fertilizer[#grapeape_plant[k].fertilizer]
...
--- Method to calculate the current water percentage for a given grapeape_plant index
--- @param k number - grapeape_plant table index
...
local calcWater = function(k)
    ...
    if #grapeape_plant[k].water == 0 then
        return 0
    else
        local last_water = grapeape_plant[k].water[#grapeape_plant[k].water]
...
--- Method to calculate the health percentage for a given grapeape_plant index
--- @param k number - grapeape_plant table index
...
local calcHealth = function(k)
    ...
    local health = 100
    local current_time = os.time()
    local planted_time = grapeape_plant[k].time
    ...
    for i=1, intervals, 1 do
        ...
        if #grapeape_plant[k].fertilizer == 0 then
            ...
            for i=1, #grapeape_plant[k].fertilizer, 1 do
                last_fertilizer = last_fertilizer < grapeape_plant[k].fertilizer[i] and last_fertilizer or grapeape_plant[k].fertilizer[i]
            end
        ...
        if #grapeape_plant[k].water == 0 then
            ...
            for i=1, #grapeape_plant[k].water, 1 do
                last_water = last_water < grapeape_plant[k].water[i] and last_water or grapeape_plant[k].water[i]
            end
...
--- Method to setup all the grapeape_plant, fetched from the database
--- @return nil
local setupPlants = function()
    local result = MySQL.Sync.fetchAll('SELECT * FROM grapeape_plant')
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
...
local destroyAllPlants = function()    
    for k, v in pairs(grapeape_plant) do
        if DoesEntityExist(k) then
            DeleteEntity(k)
            grapeape_plant[k] = nil
        end
    end
end
...
local updatePlantProp = function(k, stage)
    if not grapeape_plant[k] then return end
    if not DoesEntityExist(k) then return end
    local ModelHash = Shared.WeedProps[stage]
    DeleteEntity(k)
    local plant = CreateObjectNoOffset(ModelHash, grapeape_plant[k].coords.x, grapeape_plant[k].coords.y, grapeape_plant[k].coords.z + Shared.ObjectZOffset, true, true, false)
    FreezeEntityPosition(plant, true)
    grapeape_plant[plant] = grapeape_plant[k]
    grapeape_plant[k] = nil
end
    ...
    if Shared.ClearOnStartup then
        ...
        for k, v in pairs(grapeape_plant) do
            if calcHealth(k) == 0 then
                DeleteEntity(k)
                MySQL.query('DELETE from grapeape_plant WHERE id = :id', {
                    ['id'] = grapeape_plant[k].id
                })
                grapeape_plant[k] = nil
            end
        end
...
RegisterNetEvent('zero-masterkush:server:ClearPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not grapeape_plant[entity] then return end
    if #(GetEntityCoords(GetPlayerPed(source)) - grapeape_plant[entity].coords) > 10 then return end
    if calcHealth(entity) ~= 0 then return end

    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        MySQL.query('DELETE from grapeape_plant WHERE id = :id', {
            ['id'] = grapeape_plant[entity].id
        })
        grapeape_plant[entity] = nil
    end
end)

RegisterNetEvent('zero-masterkush:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not grapeape_plant[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(GetEntityCoords(GetPlayerPed(src)) - grapeape_plant[entity].coords) > 10 then return end
    if calcGrowth(entity) ~= 100 then return end

    if DoesEntityExist(entity) then
        local health = calcHealth(entity)
        if grapeape_plant[entity].gender == 'female' then
...
-- RegisterNetEvent('zero-masterkush:server:PoliceDestroy', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not grapeape_plant[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         TriggerClientEvent('zero-masterkush:client:FireGoBrrrrrrr', -1, grapeape_plant[entity].coords)
...
RegisterNetEvent('zero-masterkush:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not grapeape_plant[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        ...
        grapeape_plant[entity].water[#grapeape_plant[entity].water + 1] = os.time()
        MySQL.update('UPDATE grapeape_plant SET water = (:water) WHERE id = (:id)', {
            ['water'] = json.encode(grapeape_plant[entity].water),
            ['id'] = grapeape_plant[entity].id,
        })
    end
end)

RegisterNetEvent('zero-masterkush:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not grapeape_plant[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        ...
        grapeape_plant[entity].fertilizer[#grapeape_plant[entity].fertilizer + 1] = os.time()
        MySQL.update('UPDATE grapeape_plant SET fertilizer = (:fertilizer) WHERE id = (:id)', {
            ['fertilizer'] = json.encode(grapeape_plant[entity].fertilizer),
            ['id'] = grapeape_plant[entity].id,
        })
    end
end)

-- RegisterNetEvent('zero-masterkush:server:AddMaleSeed', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not grapeape_plant[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         grapeape_plant[entity].gender = 'male'
--         MySQL.update('UPDATE grapeape_plant SET gender = (:gender) WHERE id = (:id)', {
--             ['gender'] = grapeape_plant[entity].gender,
--             ['id'] = grapeape_plant[entity].id,
--         })
--     end
-- end)

RegisterNetEvent('zero-masterkush:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) then
        ...
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into grapeape_plant (coords, time, fertilizer, water, gender) VALUES (:coords, :time, :fertilizer, :water, :gender)', {
...
RegisterNetEvent('zero-masterkush:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('zero-masterkush:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not grapeape_plant[entity] then cb(nil) return end
    local temp = {
        id = grapeape_plant[entity].id,
        coords = grapeape_plant[entity].coords,
        time = grapeape_plant[entity].time,
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("zero-masterkush:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("zero-masterkush:client:OpenFillWaterMenu", src)
end)
