...
RegisterNetEvent('zero-cucumber:server:ClearPlant', function(netId)
...
RegisterNetEvent('zero-cucumber:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cucumberplants[entity] then return end
    local src = source
...
-- RegisterNetEvent('zero-cucumber:server:PoliceDestroy', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not cucumberplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         TriggerClientEvent('zero-cucumber:client:FireGoBrrrrrrr', -1, cucumberplants[entity].coords)
...
RegisterNetEvent('zero-cucumber:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cucumberplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('zero-cucumber:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not cucumberplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
-- RegisterNetEvent('zero-cucumber:server:AddMaleSeed', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not cucumberplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('zero-cucumber:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('zero-cucumber:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('zero-cucumber:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("zero-cucumber:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("zero-cucumber:client:OpenFillWaterMenu", src)
end)
