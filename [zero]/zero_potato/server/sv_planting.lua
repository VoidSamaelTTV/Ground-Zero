...
--- Method to calculate the growth percentage for a given potatoplants index
--- @param k number - potatoplants table index
--- @return retval number - growth index [0-100]
local calcGrowth = function(k)
    if not potatoplants[k] then return false end
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
    local progress = os.difftime(current_time, potatoplants[k].time)
...
--- Method to calculate the current fertilizer percentage for a given potatoplants index
--- @param k number - potatoplants table index
...
local calcFertilizer = function(k)
    ...
    if #potatoplants[k].fertilizer == 0 then
        return 0
    else
        local last_fertilizer = potatoplants[k].fertilizer[#potatoplants[k].fertilizer]
...
--- Method to calculate the current water percentage for a given potatoplants index
--- @param k number - potatoplants table index
...
local calcWater = function(k)
    ...
    if #potatoplants[k].water == 0 then
        return 0
    else
        local last_water = potatoplants[k].water[#potatoplants[k].water]
...
--- Method to calculate the health percentage for a given potatoplants index
--- @param k number - potatoplants table index
...
local calcHealth = function(k)
    ...
    local health = 100
    local current_time = os.time()
    local planted_time = potatoplants[k].time
    ...
    for i=1, intervals, 1 do
        ...
        if #potatoplants[k].fertilizer == 0 then
            ...
            for i=1, #potatoplants[k].fertilizer, 1 do
                last_fertilizer = last_fertilizer < potatoplants[k].fertilizer[i] and last_fertilizer or potatoplants[k].fertilizer[i]
            end
        ...
        if #potatoplants[k].water == 0 then
            ...
            for i=1, #potatoplants[k].water, 1 do
                last_water = last_water < potatoplants[k].water[i] and last_water or potatoplants[k].water[i]
            end
...
--- Method to setup all the potatoplants, fetched from the database
--- @return nil
local setupPlants = function()
    local result = MySQL.Sync.fetchAll('SELECT * FROM potatoplants')
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
...
local updatePlantProp = function(k, stage)
    if not potatoplants[k] then return end
    if not DoesEntityExist(k) then return end
    local ModelHash = Shared.WeedProps[stage]
    DeleteEntity(k)
    local plant = CreateObjectNoOffset(ModelHash, potatoplants[k].coords.x, potatoplants[k].coords.y, potatoplants[k].coords.z + Shared.ObjectZOffset, true, true, false)
    FreezeEntityPosition(plant, true)
    potatoplants[plant] = potatoplants[k]
    potatoplants[k] = nil
end
    ...
    if Shared.ClearOnStartup then
        ...
        for k, v in pairs(potatoplants) do
            if calcHealth(k) == 0 then
                DeleteEntity(k)
                MySQL.query('DELETE from potatoplants WHERE id = :id', {
                    ['id'] = potatoplants[k].id
                })
                potatoplants[k] = nil
            end
        end
...
RegisterNetEvent('zero-potato:server:ClearPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not potatoplants[entity] then return end
    if #(GetEntityCoords(GetPlayerPed(source)) - potatoplants[entity].coords) > 10 then return end
    if calcHealth(entity) ~= 0 then return end

    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        MySQL.query('DELETE from potatoplants WHERE id = :id', {
            ['id'] = potatoplants[entity].id
        })
        potatoplants[entity] = nil
    end
end)

RegisterNetEvent('zero-potato:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not potatoplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(GetEntityCoords(GetPlayerPed(src)) - potatoplants[entity].coords) > 10 then return end
    if calcGrowth(entity) ~= 100 then return end

    if DoesEntityExist(entity) then
        local health = calcHealth(entity)
        if potatoplants[entity].gender == 'female' then
...
-- RegisterNetEvent('zero-potato:server:PoliceDestroy', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not potatoplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         TriggerClientEvent('zero-potato:client:FireGoBrrrrrrr', -1, potatoplants[entity].coords)
...
RegisterNetEvent('zero-potato:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not potatoplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        ...
        potatoplants[entity].water[#potatoplants[entity].water + 1] = os.time()
        MySQL.update('UPDATE potatoplants SET water = (:water) WHERE id = (:id)', {
            ['water'] = json.encode(potatoplants[entity].water),
            ['id'] = potatoplants[entity].id,
        })
    end
end)

RegisterNetEvent('zero-potato:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not potatoplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        ...
        potatoplants[entity].fertilizer[#potatoplants[entity].fertilizer + 1] = os.time()
        MySQL.update('UPDATE potatoplants SET fertilizer = (:fertilizer) WHERE id = (:id)', {
            ['fertilizer'] = json.encode(potatoplants[entity].fertilizer),
            ['id'] = potatoplants[entity].id,
        })
    end
end)

-- RegisterNetEvent('zero-potato:server:AddMaleSeed', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not potatoplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         potatoplants[entity].gender = 'male'
--         MySQL.update('UPDATE potatoplants SET gender = (:gender) WHERE id = (:id)', {
--             ['gender'] = potatoplants[entity].gender,
--             ['id'] = potatoplants[entity].id,
--         })
--     end
-- end)

RegisterNetEvent('zero-potato:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) then
        ...
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into potatoplants (coords, time, fertilizer, water, gender) VALUES (:coords, :time, :fertilizer, :water, :gender)', {
...
RegisterNetEvent('zero-potato:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('zero-potato:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not potatoplants[entity] then cb(nil) return end
    local temp = {
        id = potatoplants[entity].id,
        coords = potatoplants[entity].coords,
        time = potatoplants[entity].time,
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("zero-potato:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("zero-potato:client:OpenFillWaterMenu", src)
end)
