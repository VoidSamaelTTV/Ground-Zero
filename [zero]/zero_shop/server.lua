local DiscordWebhook = 'https://discord.com/api/webhooks/1053079152640655411/_mwVRGRLPsrDnCc4uToJoLr5O7UOoCgrLuHRKjwu42KOa3E25acwd8dMLSX7xB0f4dae'
local redeemedCars = {}
...
local function SetGang(source, gang, rank)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.SetGang(gang, rank)
end
					...
					for _, v in pairs (Config.Packages) do
						if v.PackageName == i then
							for j = 1, #v.Items, 1 do
								...
								elseif counter.type == 'gang' then
									SetGang(source, counter.name, counter.rank)
								elseif counter.type == 'job' then
									SetJob(source, counter.name, counter.rank)
								end
...
RegisterNetEvent('nass_tebexstore:setVehicle', function (vehicleProps)
	...
	elseif Config.Framework == "QB" then
		local player = QBCore.Functions.GetPlayer(src)
		MySQL.insert('INSERT INTO player_vehicles (license, citizenid, plate, hash, mods, vehicle, garage, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
		{
