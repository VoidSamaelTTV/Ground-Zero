...
--- Method to calculate the growth percentage for a given skunk_plant index
--- @param k number - skunk_plant table index
--- @return retval number - growth index [0-100]
local calcGrowth = function(k)
    if not skunk_plant[k] then return false end
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
    local progress = os.difftime(current_time, skunk_plant[k].time)
...
--- Method to calculate the current fertilizer percentage for a given skunk_plant index
--- @param k number - skunk_plant table index
...
local calcFertilizer = function(k)
    ...
    if #skunk_plant[k].fertilizer == 0 then
        return 0
    else
        local last_fertilizer = skunk_plant[k].fertilizer[#skunk_plant[k].fertilizer]
...
--- Method to calculate the current water percentage for a given skunk_plant index
--- @param k number - skunk_plant table index
...
local calcWater = function(k)
    ...
    if #skunk_plant[k].water == 0 then
        return 0
    else
        local last_water = skunk_plant[k].water[#skunk_plant[k].water]
...
--- Method to calculate the health percentage for a given skunk_plant index
--- @param k number - skunk_plant table index
...
local calcHealth = function(k)
    ...
    local health = 100
    local current_time = os.time()
    local planted_time = skunk_plant[k].time
    ...
    for i=1, intervals, 1 do
        ...
        if #skunk_plant[k].fertilizer == 0 then
            ...
            for i=1, #skunk_plant[k].fertilizer, 1 do
                last_fertilizer = last_fertilizer < skunk_plant[k].fertilizer[i] and last_fertilizer or skunk_plant[k].fertilizer[i]
            end
        ...
        if #skunk_plant[k].water == 0 then
            ...
            for i=1, #skunk_plant[k].water, 1 do
                last_water = last_water < skunk_plant[k].water[i] and last_water or skunk_plant[k].water[i]
            end
...
--- Method to setup all the skunk_plant, fetched from the database
--- @return nil
local setupPlants = function()
    local result = MySQL.Sync.fetchAll('SELECT * FROM skunk_plant')
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
...
local destroyAllPlants = function()    
    for k, v in pairs(skunk_plant) do
...
local updatePlantProp = function(k, stage)
    if not skunk_plant[k] then return end
    if not DoesEntityExist(k) then return end
    local ModelHash = Shared.WeedProps[stage]
    DeleteEntity(k)
    local plant = CreateObjectNoOffset(ModelHash, skunk_plant[k].coords.x, skunk_plant[k].coords.y, skunk_plant[k].coords.z + Shared.ObjectZOffset, true, true, false)
    FreezeEntityPosition(plant, true)
    skunk_plant[plant] = skunk_plant[k]
    skunk_plant[k] = nil
end
    ...
    if Shared.ClearOnStartup then
        ...
        for k, v in pairs(skunk_plant) do
            if calcHealth(k) == 0 then
                DeleteEntity(k)
                MySQL.query('DELETE from skunk_plant WHERE id = :id', {
                    ['id'] = skunk_plant[k].id
                })
                skunk_plant[k] = nil
            end
...
RegisterNetEvent('zero-skunk:server:ClearPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not skunk_plant[entity] then return end
    if #(GetEntityCoords(GetPlayerPed(source)) - skunk_plant[entity].coords) > 10 then return end
    if calcHealth(entity) ~= 0 then return end

    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        MySQL.query('DELETE from skunk_plant WHERE id = :id', {
            ['id'] = skunk_plant[entity].id
        })
        skunk_plant[entity] = nil
    end
end)

RegisterNetEvent('zero-skunk:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not skunk_plant[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(GetEntityCoords(GetPlayerPed(src)) - skunk_plant[entity].coords) > 10 then return end
    if calcGrowth(entity) ~= 100 then return end

    if DoesEntityExist(entity) then
        local health = calcHealth(entity)
        if skunk_plant[entity].gender == 'female' then
...
-- RegisterNetEvent('zero-skunk:server:PoliceDestroy', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not skunk_plant[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         TriggerClientEvent('zero-skunk:client:FireGoBrrrrrrr', -1, skunk_plant[entity].coords)
...
RegisterNetEvent('zero-skunk:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not skunk_plant[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        ...
        skunk_plant[entity].water[#skunk_plant[entity].water + 1] = os.time()
        MySQL.update('UPDATE skunk_plant SET water = (:water) WHERE id = (:id)', {
            ['water'] = json.encode(skunk_plant[entity].water),
            ['id'] = skunk_plant[entity].id,
        })
    end
end)

RegisterNetEvent('zero-skunk:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not skunk_plant[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        ...
        skunk_plant[entity].fertilizer[#skunk_plant[entity].fertilizer + 1] = os.time()
        MySQL.update('UPDATE skunk_plant SET fertilizer = (:fertilizer) WHERE id = (:id)', {
            ['fertilizer'] = json.encode(skunk_plant[entity].fertilizer),
            ['id'] = skunk_plant[entity].id,
        })
    end
end)

-- RegisterNetEvent('zero-skunk:server:AddMaleSeed', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not skunk_plant[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         skunk_plant[entity].gender = 'male'
--         MySQL.update('UPDATE skunk_plant SET gender = (:gender) WHERE id = (:id)', {
--             ['gender'] = skunk_plant[entity].gender,
--             ['id'] = skunk_plant[entity].id,
--         })
--     end
-- end)

RegisterNetEvent('zero-skunk:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) then
        ...
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into skunk_plant (coords, time, fertilizer, water, gender) VALUES (:coords, :time, :fertilizer, :water, :gender)', {
...
RegisterNetEvent('zero-skunk:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('zero-skunk:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not skunk_plant[entity] then cb(nil) return end
    local temp = {
        id = skunk_plant[entity].id,
        coords = skunk_plant[entity].coords,
        time = skunk_plant[entity].time,
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("zero-skunk:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("zero-skunk:client:OpenFillWaterMenu", src)
end)
