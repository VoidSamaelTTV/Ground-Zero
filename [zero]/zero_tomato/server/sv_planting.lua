...
--- Method to calculate the growth percentage for a given tomatoplants index
--- @param k number - tomatoplants table index
--- @return retval number - growth index [0-100]
local calcGrowth = function(k)
    if not tomatoplants[k] then return false end
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
    local progress = os.difftime(current_time, tomatoplants[k].time)
...
--- Method to calculate the current fertilizer percentage for a given tomatoplants index
--- @param k number - tomatoplants table index
...
local calcFertilizer = function(k)
    ...
    if #tomatoplants[k].fertilizer == 0 then
        return 0
    else
        local last_fertilizer = tomatoplants[k].fertilizer[#tomatoplants[k].fertilizer]
...
--- Method to calculate the current water percentage for a given tomatoplants index
--- @param k number - tomatoplants table index
...
local calcWater = function(k)
    ...
    if #tomatoplants[k].water == 0 then
        return 0
    else
        local last_water = tomatoplants[k].water[#tomatoplants[k].water]
...
--- Method to calculate the health percentage for a given tomatoplants index
--- @param k number - tomatoplants table index
...
local calcHealth = function(k)
    ...
    local health = 100
    local current_time = os.time()
    local planted_time = tomatoplants[k].time
    ...
    for i=1, intervals, 1 do
        ...
        if #tomatoplants[k].fertilizer == 0 then
            ...
            for i=1, #tomatoplants[k].fertilizer, 1 do
                last_fertilizer = last_fertilizer < tomatoplants[k].fertilizer[i] and last_fertilizer or tomatoplants[k].fertilizer[i]
            end
        ...
        if #tomatoplants[k].water == 0 then
            ...
            for i=1, #tomatoplants[k].water, 1 do
                last_water = last_water < tomatoplants[k].water[i] and last_water or tomatoplants[k].water[i]
            end
...
--- Method to setup all the tomatoplants, fetched from the database
--- @return nil
local setupPlants = function()
    local result = MySQL.Sync.fetchAll('SELECT * FROM tomatoplants')
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
...
local updatePlantProp = function(k, stage)
    if not tomatoplants[k] then return end
    if not DoesEntityExist(k) then return end
    local ModelHash = Shared.WeedProps[stage]
    DeleteEntity(k)
    local plant = CreateObjectNoOffset(ModelHash, tomatoplants[k].coords.x, tomatoplants[k].coords.y, tomatoplants[k].coords.z + Shared.ObjectZOffset, true, true, false)
    FreezeEntityPosition(plant, true)
    tomatoplants[plant] = tomatoplants[k]
    tomatoplants[k] = nil
end
    ...
    if Shared.ClearOnStartup then
        ...
        for k, v in pairs(tomatoplants) do
            if calcHealth(k) == 0 then
                DeleteEntity(k)
                MySQL.query('DELETE from tomatoplants WHERE id = :id', {
                    ['id'] = tomatoplants[k].id
                })
                tomatoplants[k] = nil
            end
        end
...
RegisterNetEvent('zero-tomato:server:ClearPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not tomatoplants[entity] then return end
    if #(GetEntityCoords(GetPlayerPed(source)) - tomatoplants[entity].coords) > 10 then return end
    if calcHealth(entity) ~= 0 then return end

    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        MySQL.query('DELETE from tomatoplants WHERE id = :id', {
            ['id'] = tomatoplants[entity].id
        })
        tomatoplants[entity] = nil
    end
end)

RegisterNetEvent('zero-tomato:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not tomatoplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(GetEntityCoords(GetPlayerPed(src)) - tomatoplants[entity].coords) > 10 then return end
    if calcGrowth(entity) ~= 100 then return end

    if DoesEntityExist(entity) then
        local health = calcHealth(entity)
        if tomatoplants[entity].gender == 'female' then
...
-- RegisterNetEvent('zero-tomato:server:PoliceDestroy', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not tomatoplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         TriggerClientEvent('zero-tomato:client:FireGoBrrrrrrr', -1, tomatoplants[entity].coords)
...
RegisterNetEvent('zero-tomato:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not tomatoplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        ...
        tomatoplants[entity].water[#tomatoplants[entity].water + 1] = os.time()
        MySQL.update('UPDATE tomatoplants SET water = (:water) WHERE id = (:id)', {
            ['water'] = json.encode(tomatoplants[entity].water),
            ['id'] = tomatoplants[entity].id,
        })
    end
end)

RegisterNetEvent('zero-tomato:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not tomatoplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        ...
        tomatoplants[entity].fertilizer[#tomatoplants[entity].fertilizer + 1] = os.time()
        MySQL.update('UPDATE tomatoplants SET fertilizer = (:fertilizer) WHERE id = (:id)', {
            ['fertilizer'] = json.encode(tomatoplants[entity].fertilizer),
            ['id'] = tomatoplants[entity].id,
        })
    end
end)

-- RegisterNetEvent('zero-tomato:server:AddMaleSeed', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not tomatoplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         tomatoplants[entity].gender = 'male'
--         MySQL.update('UPDATE tomatoplants SET gender = (:gender) WHERE id = (:id)', {
--             ['gender'] = tomatoplants[entity].gender,
--             ['id'] = tomatoplants[entity].id,
--         })
--     end
-- end)

RegisterNetEvent('zero-tomato:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) then
        ...
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into tomatoplants (coords, time, fertilizer, water, gender) VALUES (:coords, :time, :fertilizer, :water, :gender)', {
...
RegisterNetEvent('zero-tomato:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('zero-tomato:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not tomatoplants[entity] then cb(nil) return end
    local temp = {
        id = tomatoplants[entity].id,
        coords = tomatoplants[entity].coords,
        time = tomatoplants[entity].time,
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("zero-tomato:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("zero-tomato:client:OpenFillWaterMenu", src)
end)
