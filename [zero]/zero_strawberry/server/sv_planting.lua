...
local strawberryplants = {}

--- Functions

--- Method to calculate the growth percentage for a given strawberryplants index
--- @param k number - strawberryplants table index
--- @return retval number - growth index [0-100]
local calcGrowth = function(k)
    if not strawberryplants[k] then return false end
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
    local progress = os.difftime(current_time, strawberryplants[k].time)
...
--- Method to calculate the current fertilizer percentage for a given strawberryplants index
--- @param k number - strawberryplants table index
...
local calcFertilizer = function(k)
    ...
    if #strawberryplants[k].fertilizer == 0 then
        return 0
    else
        local last_fertilizer = strawberryplants[k].fertilizer[#strawberryplants[k].fertilizer]
...
--- Method to calculate the current water percentage for a given strawberryplants index
--- @param k number - strawberryplants table index
...
local calcWater = function(k)
    ...
    if #strawberryplants[k].water == 0 then
        return 0
    else
        local last_water = strawberryplants[k].water[#strawberryplants[k].water]
...
--- Method to calculate the health percentage for a given strawberryplants index
--- @param k number - strawberryplants table index
...
local calcHealth = function(k)
    ...
    local health = 100
    local current_time = os.time()
    local planted_time = strawberryplants[k].time
    ...
    for i=1, intervals, 1 do
        ...
        if #strawberryplants[k].fertilizer == 0 then
            ...
            for i=1, #strawberryplants[k].fertilizer, 1 do
                last_fertilizer = last_fertilizer < strawberryplants[k].fertilizer[i] and last_fertilizer or strawberryplants[k].fertilizer[i]
            end
        ...
        if #strawberryplants[k].water == 0 then
            ...
            for i=1, #strawberryplants[k].water, 1 do
                last_water = last_water < strawberryplants[k].water[i] and last_water or strawberryplants[k].water[i]
            end
...
--- Method to setup all the strawberryplants, fetched from the database
--- @return nil
local setupPlants = function()
    local result = MySQL.Sync.fetchAll('SELECT * FROM strawberryplants')
    local current_time = os.time()
    local growTime = Shared.GrowTime * 60
...
local destroyAllPlants = function()    
    for k, v in pairs(strawberryplants) do
        if DoesEntityExist(k) then
            DeleteEntity(k)
            strawberryplants[k] = nil
        end
    end
end
...
local updatePlantProp = function(k, stage)
    if not strawberryplants[k] then return end
    if not DoesEntityExist(k) then return end
    local ModelHash = Shared.WeedProps[stage]
    DeleteEntity(k)
    local plant = CreateObjectNoOffset(ModelHash, strawberryplants[k].coords.x, strawberryplants[k].coords.y, strawberryplants[k].coords.z + Shared.ObjectZOffset, true, true, false)
    FreezeEntityPosition(plant, true)
    strawberryplants[plant] = strawberryplants[k]
    strawberryplants[k] = nil
end
...
updatePlants = function()
    for k, v in pairs(strawberryplants) do
        ...
        if stage ~= v.stage then
            strawberryplants[k].stage = stage
    ...
    if Shared.ClearOnStartup then
        ...
        for k, v in pairs(strawberryplants) do
            if calcHealth(k) == 0 then
                DeleteEntity(k)
                MySQL.query('DELETE from strawberryplants WHERE id = :id', {
                    ['id'] = strawberryplants[k].id
                })
                strawberryplants[k] = nil
            end
        end
...
RegisterNetEvent('zero-strawberry:server:ClearPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not strawberryplants[entity] then return end
    if #(GetEntityCoords(GetPlayerPed(source)) - strawberryplants[entity].coords) > 10 then return end
    if calcHealth(entity) ~= 0 then return end

    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        MySQL.query('DELETE from strawberryplants WHERE id = :id', {
            ['id'] = strawberryplants[entity].id
        })
        strawberryplants[entity] = nil
    end
end)

RegisterNetEvent('zero-strawberry:server:HarvestPlant', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not strawberryplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(GetEntityCoords(GetPlayerPed(src)) - strawberryplants[entity].coords) > 10 then return end
    if calcGrowth(entity) ~= 100 then return end

    if DoesEntityExist(entity) then
        local health = calcHealth(entity)
        if strawberryplants[entity].gender == 'female' then
...
-- RegisterNetEvent('zero-strawberry:server:PoliceDestroy', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not strawberryplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         TriggerClientEvent('zero-strawberry:client:FireGoBrrrrrrr', -1, strawberryplants[entity].coords)
...
RegisterNetEvent('zero-strawberry:server:GiveWater', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not strawberryplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FullCanItem, 1, false) then
        ...
        strawberryplants[entity].water[#strawberryplants[entity].water + 1] = os.time()
        MySQL.update('UPDATE strawberryplants SET water = (:water) WHERE id = (:id)', {
            ['water'] = json.encode(strawberryplants[entity].water),
            ['id'] = strawberryplants[entity].id,
        })
    end
end)

RegisterNetEvent('zero-strawberry:server:GiveFertilizer', function(netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not strawberryplants[entity] then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(Shared.FertilizerItem, 1, false) then
        ...
        strawberryplants[entity].fertilizer[#strawberryplants[entity].fertilizer + 1] = os.time()
        MySQL.update('UPDATE strawberryplants SET fertilizer = (:fertilizer) WHERE id = (:id)', {
            ['fertilizer'] = json.encode(strawberryplants[entity].fertilizer),
            ['id'] = strawberryplants[entity].id,
        })
    end
end)

-- RegisterNetEvent('zero-strawberry:server:AddMaleSeed', function(netId)
--     local entity = NetworkGetEntityFromNetworkId(netId)
--     if not strawberryplants[entity] then return end
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
--         strawberryplants[entity].gender = 'male'
--         MySQL.update('UPDATE strawberryplants SET gender = (:gender) WHERE id = (:id)', {
--             ['gender'] = strawberryplants[entity].gender,
--             ['id'] = strawberryplants[entity].id,
--         })
--     end
-- end)

RegisterNetEvent('zero-strawberry:server:CreateNewPlant', function(coords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(Shared.FemaleSeed, 1) then
        ...
        FreezeEntityPosition(plant, true)
        local time = os.time()
        MySQL.insert('INSERT into strawberryplants (coords, time, fertilizer, water, gender) VALUES (:coords, :time, :fertilizer, :water, :gender)', {
...
RegisterNetEvent('zero-strawberry:server:GetFullWateringCan', function(netId)
...
QBCore.Functions.CreateCallback('zero-strawberry:server:GetPlantData', function(source, cb, netId)
    local entity = NetworkGetEntityFromNetworkId(netId)
    if not strawberryplants[entity] then cb(nil) return end
    local temp = {
        id = strawberryplants[entity].id,
        coords = strawberryplants[entity].coords,
        time = strawberryplants[entity].time,
...
QBCore.Functions.CreateUseableItem(Shared.FemaleSeed, function(source)
    ...
    if tub ~= nil then
        TriggerClientEvent("zero-strawberry:client:UseWeedSeed", source)
    else
...
QBCore.Functions.CreateUseableItem(Shared.EmptyCanItem, function(source)
    local src = source
    TriggerClientEvent("zero-strawberry:client:OpenFillWaterMenu", src)
end)
