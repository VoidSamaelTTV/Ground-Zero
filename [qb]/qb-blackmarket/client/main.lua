...
--         for key, value in pairs(Config.bmLocation) do
--             exports['qb-target']:AddBoxZone('BlackMarket'..key, value.coords, value.length, value.width, {
...
--             })
--         end
--         local BlackMarketCombo = ComboZone:Create( zone, { name = 'NewBlackMarketCombo', debugPoly = false })
--         BlackMarketCombo:onPlayerInOut(function(isPointInside)
        ...
        if GetClockHours() >= Config.TimeOpen and GetClockHours() <= Config.TimeClosed then
            local BlackMarket = {
                ...
                },
                {
                    ...
                    params = {
                        event = 'qb-blackmarket:client:openPawn',
                        args = {
                            items = Config.bmItems
                        }
                    }
                }
            }
...
RegisterNetEvent('qb-blackmarket:client:openPawn', function(data)
    QBCore.Functions.TriggerCallback('qb-blackmarket:server:getInv', function(inventory)
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    pawnMenu[#pawnMenu + 1] = {
                        ...
                        params = {
                            event = 'qb-blackmarket:client:bmItems',
                            args = {
...
RegisterNetEvent('qb-blackmarket:client:openMelt', function(data)
    QBCore.Functions.TriggerCallback('qb-blackmarket:server:getInv', function(inventory)
        local PlyInv = inventory
        local meltMenu = {
            {
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    meltMenu[#meltMenu + 1] = {
                        ...
                        params = {
                            event = 'qb-blackmarket:client:meltItems',
                            args = {
                                label = QBCore.Shared.Items[v.name].label,
...
RegisterNetEvent('qb-blackmarket:client:bmItems', function(item)
    ...
    if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            TriggerServerEvent('qb-blackmarket:server:sellbmItems', item.name, sellingItem.amount, item.price)
        else
...
RegisterNetEvent('qb-blackmarket:client:meltItems', function(item)
    ...
    if meltingItem then
        ...
        if meltingItem.amount ~= nil then
            if tonumber(meltingItem.amount) > 0 then
                TriggerServerEvent('qb-blackmarket:server:meltItemRemove', item.name, meltingItem.amount, item)
...
RegisterNetEvent('qb-blackmarket:client:startMelting', function(item, meltingAmount, meltTimees)
...
RegisterNetEvent('qb-blackmarket:client:resetPickup', function()
    canTake = false
end)