...
RegisterNetEvent('wais:player:deathblips', function(attacker, deader)
    local aname = GetPlayerName(attacker)
    local dname = GetPlayerName(deader)
    local dcoords = GetEntityCoords(GetPlayerPed(deader))

    TriggerClientEvent('wais:attacker:send:coords', attacker, dcoords, dname)
    TriggerClientEvent('wais:deader:send:coords', deader, dcoords)
end)
        ...
        if resultData ~= nil then
            ...
            if newversion == "error" or newversion == "dontfind" then
                ...
                if version == newversion then
                    ...
                    CreateThread(function()
                        while true do
                            print('^3There is a new version of the script you are using. ^3You can get the new version via Ayazawai#3900 or discord.gg/0resmon discord. [Script: ^1'..script..'^3 Version: ^1v'..version.. "^3 New version: ^1v"..newversion.. "^3]")
                            Wait(60 * 1000)
                        end
                    end)
                end
