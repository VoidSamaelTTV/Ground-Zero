...
function updateCar(vehicle)
	if DoesEntityExist(vehicle) and vehicle ~= 0 and vehicle ~= nil then 									-- Check if somehow the vehicle doesn't exist
		local newProps = getVehicleProperties(vehicle) 														-- Grab properties of the vehicle
		TriggerServerEvent("jim-mechanic:server:updateCar", VehToNet(vehicle), newProps) 					-- Update everyone who can see the entity with the new changes (helps syncing)
		updateDelay[vehicle] = { delay = (Config.Overrides.updateServerDelay / 10) or 2, mods = newProps } 	-- new/refresh cooldown for the mods to be saved
...
RegisterNetEvent("jim-mechanic:forceProperties", function(vehicle, props, src) 								-- This forces updates of the vehicle from the person who updated it
	if src ~= GetPlayerServerId(PlayerId()) then															-- Skip the player who modded it (stops the exploding windows)
		local netID = ensureNetToVeh(vehicle)																-- Check if it exists for the player
		if netID ~= 0 and DoesEntityExist(netID) then
			setVehicleProperties(netID, props)																-- Set the collected mods to the vehicle for the player that recieved them
		end
	end
end)

CreateThread(function() 																					-- constant loop for counting down saving to database
	while true do
		for veh in pairs(updateDelay) do 																	-- start looping the `updateDelay` table
			if updateDelay[veh].delay then updateDelay[veh].delay -= 1 										-- count down the cooldown
				if updateDelay[veh].delay <= 0 then 														-- if its 0 or lower, start saving
					if DoesEntityExist(veh) and veh ~= 0 and veh ~= nil then 								-- if the entity no longer "exists" skip it
						if IsVehicleOwned(updateDelay[veh].mods.plate) then									-- If vehicle is owned then
							forceUpdateCar(veh, updateDelay[veh].mods)										-- send data to database that was picked at the time of modding
						end
					end
					updateDelay[veh] = nil 																	-- remove data from table when done
				end
			end
		end
		Wait(10000)
...
function forceUpdateCar(vehicle, mods)
	...
	TriggerServerEvent('jim-mechanic:updateVehicle', mods, trim(mods.plate))								-- Event to save stored mod data to database
