$('.wrapper, .inmenu, .left-wrapper, .invites, .creates').hide();
...
window.addEventListener('message', function (event) {
    ...
    switch (e.type) {
        ...
        case "CLOSE_MENU":
            ...
            $('.wrapper, .inmenu, .invites, .creates').hide();
            break;
...
document.onkeyup = function(data){
    if (data.key == "Escape"){
        if (!cooldown) {
            $.post('http://wais-squad/close', JSON.stringify({}));
        }
    }
}
...
$(document).on('click', '.create-squads', function(){
    if (!cooldown) {
        ...
        if (squadname.length > 1 && squadname != "" && squadname != null) {
            $.post('http://wais-squad/createsquad', JSON.stringify({name: squadname, isprivate: private}), function(created) {
                if (created) {
                    $('#newsquadname').val("");
...
$(document).on('click', '.leave-squad', function(){
    if (!cooldown) {
        cooldown = true
        let squadname = $(this).data('roomname');
        $.post('http://wais-squad/leavesquad', JSON.stringify({name: squadname}), function(leaved) {
            if (leaved) {
                insquads = false;
                UIbuilder();
            }
...
$(document).on('click', '#join', function(){
    if (!cooldown) {
        ...
        $.post('http://wais-squad/joinsquad', JSON.stringify({name: squadname}), function(joined) {
            if (joined) {
                insquads = true;
                UIbuilder();
            }else {
...
$(document).on('click', '#kick', function(){
    if (!cooldown) {
        ...
        $.post('http://wais-squad/kickplayer', JSON.stringify({name: squadname, target: targetid}), function(kicked) {
            if (kicked) {
                cooldown = false
            }else {
                cooldown = false
...
$(document).on('click', '.accept', function(){
    if (!cooldown) {
        ...
        let squadname = $(this).data('squadname');
        $.post('http://wais-squad/invite-accept', JSON.stringify({name: squadname}), function(joined) {
            if (joined) {
                div.remove();
                insquads = true;
                UIbuilder();
...
function loaded() {
    $.post('http://wais-squad/nuiloaded', JSON.stringify({}));
}
...
function buildRoomsFirst(rooms) {
    ...
    $.post('http://wais-squad/nuidataloaded', JSON.stringify({}));
}
...
function UIbuilder() {
    $('.wrapper').css({"display": `${insquads ? "none" : "flex"}`});
    $('.inmenu').css({"display": `${insquads ? "flex" : "none"}`});
    $('.left-wrapper').css({"display": `${insquads ? "flex" : "none"}`});
    if (!insquads) {
        $('.squads, .invites, .creates').hide();
        $(`.${lastpage}`).show();
...
function squadBuild(prop) {
    $('.inmenu, .left-wrapper').html("");
    let menu = `<div class="i-header"><span>${prop.roomname}</span><div class="leave-squad" data-roomname="${prop.roomname}"><i class="fa-solid fa-right-from-bracket"></i></div></div><div class="i-players"></div>`;
    $('.inmenu').append(menu);
