...
RegisterNUICallback('createsquad', function(data, cbj)
	QBCore.Functions.TriggerCallback('wais:createsquad', function(cbs)
		cbj(cbs)
	end, tostring(data.name), data.isprivate)
end)

RegisterNUICallback('leavesquad', function(data, cbj)
	QBCore.Functions.TriggerCallback('wais:leave:squad', function(cbs)
		cbj(cbs)
	end, tostring(data.name))
end)

RegisterNUICallback('joinsquad', function(data, cbj)
	QBCore.Functions.TriggerCallback('wais:join:squad', function(cbs)
		cbj(cbs)
	end, tostring(data.name))
end)

RegisterNUICallback('kickplayer', function(data, cbj)
	QBCore.Functions.TriggerCallback('wais:squad:kickp', function(cbs)
		cbj(cbs)
	end, tostring(data.name), tonumber(data.target))
end)

RegisterNUICallback('invite-accept', function(data, cbj)
	QBCore.Functions.TriggerCallback('wais:invite:accept', function(cbs)
		if not cbs then
			QBCore.Functions.Notify('The operation failed. Squad closed or full.', 3500)
		end
		cbj(cbs)
	end, tostring(data.name))
end)
...
function sendstat()
	for i = 1, #mysquad.players do
		...
		if (_distance < 2000 and _distance > 1) or GetPlayerServerId(PlayerId()) == mysquad.players[i].id then
